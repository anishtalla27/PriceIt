...
export function HelpTooltip({ term }: { term: string }) {
  ...
  return (
    <span className="relative inline-flex items-center ml-1 align-middle">
      ...
      {open && (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-52 rounded-xl border border-[#9BD8E2] bg-white px-3 py-2 text-[11px] font-medium text-[#2B2B2B] shadow-lg leading-relaxed pointer-events-none whitespace-normal">
          <strong className="font-bold text-[#0B7180]">{term}: </strong>
          {definition}
        </span>
      )}
    </span>
