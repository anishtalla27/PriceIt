...
Rules: at least 6 "Love it ❤️" and at least 3 "Needs work 🔧". Ratings should match the tag (Love it = 4-5, Needs work = 1-3).`;
    ...
    try {
      ...
      setReviews(reviewData.reviews.slice(0, 10));
    } catch (e) {
      console.error(e);
        ...
        <div className="w-full max-w-lg">
          ...
          {!loading && errorType === "none" && businessRating && (
            <div className="flex flex-col gap-6">
              {/* ── Overall rating ── */}
              <div className="bg-white rounded-3xl border border-[#E0EFF1] px-6 py-6 text-center shadow-sm">
                <p className="text-xs font-bold uppercase tracking-wider text-[#9BBFC3] mb-3">
                  Overall Business Rating
                </p>
                <Stars rating={businessRating.rating} size="lg" color="#F36C3D" />
                <p className="text-4xl font-extrabold text-[#2B2B2B] mt-2">
                  {businessRating.rating} / 5
                </p>
                <p className="text-[15px] text-[#5B7780] mt-3 leading-relaxed max-w-sm mx-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white rounded-2xl border border-[#E0EFF1] flex flex-col items-center justify-center gap-2 py-8 text-center px-3">
                  <span className="text-3xl">🥧</span>
                  <p className="text-xs font-bold text-[#9BBFC3] uppercase tracking-wider">Cost Breakdown</p>
                  <p className="text-sm font-bold text-[#2B2B2B]">
                    ${costPerUnit.toFixed(2)} total per product
                  </p>
                  <p className="text-[11px] text-[#7B9EA3]">
                    ${totalVariablePerUnit.toFixed(2)} variable + ${fixedPerUnit.toFixed(2)} fixed
                  </p>
                </div>

                <div className="bg-white rounded-2xl border border-[#E0EFF1] flex flex-col items-center justify-center gap-2 py-8 text-center px-3">
                  <span className="text-3xl">📈</span>
                  <p className="text-xs font-bold text-[#9BBFC3] uppercase tracking-wider">Profit Snapshot</p>
                  <p className="text-sm font-bold text-[#2B2B2B]">
                    {profitPerUnit >= 0 ? "+" : ""}${profitPerUnit.toFixed(2)} per sale
                  </p>
                  <p className="text-[11px] text-[#7B9EA3]">{marginPct.toFixed(1)}% margin</p>
                </div>
              </div>
              ...
              {reviews.length > 0 && (
                <div>
                  <h2 className="text-base font-extrabold text-[#2B2B2B] mb-3">
