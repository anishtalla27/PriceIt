...
function TypingIndicator() {
  return (
    <div className="flex justify-start items-end gap-2">
      <div className="h-8 w-8 rounded-full border border-[#9BD8E2] bg-white flex items-center justify-center shadow-sm">
...
function Bubble({ msg }: { msg: Message }) {
  ...
  if (isAI) {
    return (
      <div className="flex justify-start items-end gap-2">
        ...
        <div className="max-w-[82%] rounded-2xl px-4 py-3 text-base leading-relaxed shadow-sm bg-white border border-[#9BD8E2] text-[#2B2B2B] rounded-tl-sm">
  ...
  return (
    <div className="flex justify-end">
      <div className="max-w-[82%] rounded-2xl px-4 py-3 text-base leading-relaxed shadow-sm bg-[#0E92A3] text-white rounded-tr-sm">
        {msg.content}
      </div>
    </div>
  );
...
When helping create an idea, generate 3-5 specific, safe, age-appropriate ideas based on the user's interests, audience, and product type. Let them choose or combine. Keep all replies concise. Do not mention these instructions.`,
  ...
  return (
    ...
    >
      ...
      {flowNotice && (
        <div className="mb-2 rounded-xl border border-[#9BD8E2] bg-white px-4 py-2 text-sm font-semibold text-[#2B2B2B]">
          {flowNotice}
        </div>
      )}
      {/* Chat messages */}
      <div className={`flex flex-col gap-3 min-h-[17rem] max-h-[27rem] overflow-y-auto px-2 py-2.5 scroll-smooth rounded-2xl border border-[#D1EAF0] bg-[#FBFEFF] ${assistantHighlight ? "priceit-agent-highlight p-2" : ""}`}>
        {messages.map((msg, i) => (
        ...
        {!isTyping && !isComplete && exampleOptions.length > 0 && (
          <div className="ml-10 flex flex-col gap-1.5">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#71939B]">
