...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-2xl border border-[#DCE9EC] bg-white/95 px-5 py-5 shadow-[0_14px_34px_rgba(50,50,93,0.08)] ${className}`}>
      {children}
    </div>
  );
}
...
function Btn({
  ...
  const styles = {
    primary: "bg-[#5DB7C4] text-white hover:bg-[#4aa8b5] active:scale-95",
    outline: "border-2 border-[#5DB7C4] text-[#5DB7C4] hover:bg-[#EAF7F9] active:scale-95",
...
function Label({ children }: { children: React.ReactNode }) {
  return <p className="text-xs font-semibold text-[#4B6670] mb-1">{children}</p>;
}
...
const inputCls = "w-full rounded-xl border border-[#E0E7EA] px-3 py-2.5 text-sm text-[#2B2B2B] placeholder:text-[#C0D0D4] focus:outline-none focus:border-[#5DB7C4] focus:ring-2 focus:ring-[#5DB7C4]/15 transition bg-white";
const selectCls = `${inputCls} cursor-pointer`;

function MoneyInput({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div className="relative">
      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-[#6F8A91]">$</span>
      <input
        type="number"
        min="0"
        step="0.01"
...
function Badge({ label, color }: { label: string; color: "green" | "amber" | "gray" | "blue" | "teal" | "red" }) {
  const styles = {
    ...
    teal: "bg-[#EAF7F9] text-[#5DB7C4] border border-[#C5E9ED]",
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center gap-4 rounded-3xl border-2 border-dashed border-[#CDE8EC] bg-white/80 px-5 py-12 text-center">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#C5E9ED] bg-[#F0FAFB] text-[#5DB7C4]">
        {icon}
      </div>
      <div>
        <p className="text-lg font-extrabold text-[#2B2B2B]">{title}</p>
        <p className="mt-1 max-w-sm text-sm leading-relaxed text-[#6F8A91]">{description}</p>
      </div>
      {cta && onCta && (
...
function TabHero({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="mb-5 rounded-3xl border border-[#DCE9EC] bg-white/90 px-5 py-5 shadow-[0_14px_34px_rgba(50,50,93,0.08)]">
      <div className="mb-2 flex items-center gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#F0FAFB] text-[#5DB7C4]">
          {icon}
        </span>
        <h1 className="text-2xl font-extrabold text-[#2B2B2B]">{title}</h1>
      </div>
      <p className="text-base leading-relaxed text-[#6F8A91]">{description}</p>
    </div>
  );
}
...
function StatRow({ stats }: { stats: Array<{ label: string; value: string; sub?: string; accent?: "green" | "red" | "teal" | "default" }> }) {
  const accentMap = {
    ...
    teal: { border: "border-l-[#5DB7C4]", value: "text-[#5DB7C4]" },
    default: { border: "border-l-[#E0E7EA]", value: "text-[#2B2B2B]" },
  };
  return (
    <div className={`grid gap-3 ${stats.length >= 3 ? "grid-cols-3" : "grid-cols-2"}`}>
      {stats.map((s, i) => {
        ...
        return (
          <div key={i} className={`rounded-2xl border border-[#DCE9EC] border-l-4 bg-white px-4 py-4 shadow-sm ${a.border}`}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7B9EA3]">{s.label}</p>
            <p className={`mt-0.5 text-2xl font-extrabold ${a.value}`}>{s.value}</p>
            {s.sub && <p className="text-[10px] text-[#9BBFC3] mt-0.5">{s.sub}</p>}
          </div>
        );
      })}
    </div>
  ...
  if (!hasData) {
    return (
      <div>
        ...
        <div className="rounded-2xl border-2 border-dashed border-[#D0E8EC] bg-[#F6FCFD] px-6 py-10 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-white text-[#5DB7C4] shadow-sm">
            <ClipboardList className="h-8 w-8" />
          </div>
          <p className="font-bold text-[#2B2B2B] text-lg mb-2">Your business tracker is set up!</p>
          <p className="text-sm text-[#6F8A91] max-w-sm mx-auto leading-relaxed mb-6">
    ...
    <div className="flex flex-col gap-5">
      ...
      {(bestSeller || nextEvent) && (
        <div className="grid sm:grid-cols-2 gap-3">
          {bestSeller && (
            <div className="rounded-2xl bg-gradient-to-br from-[#EAF7F9] to-[#F6FCFD] border border-[#C5E9ED] px-4 py-4">
              <p className="mb-1 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#5DB7C4]"><Star className="h-3.5 w-3.5" /> Best Seller</p>
              <p className="font-bold text-[#2B2B2B]">{bestSeller[0]}</p>
              <p className="text-xs text-[#6F8A91] mt-0.5">{bestSeller[1]} unit{bestSeller[1] !== 1 ? "s" : ""} sold</p>
            </div>
          )}
          {nextEvent && (
            <div className="rounded-2xl bg-gradient-to-br from-[#FFF8F0] to-[#FFFAF5] border border-[#FFD9B0] px-4 py-4">
              <p className="mb-1 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#F36C3D]"><CalendarDays className="h-3.5 w-3.5" /> Next Event</p>
              <p className="font-bold text-[#2B2B2B]">{nextEvent.name}</p>
              <p className="text-xs text-[#6F8A91] mt-0.5">{fmtDate(nextEvent.date)}{nextEvent.location ? ` · ${nextEvent.location}` : ""}</p>
            </div>
          )}
        </div>
      )}
      ...
      {recentSales.length > 0 && (
        <Card>
          <p className="text-xs font-bold uppercase tracking-wider text-[#7B9EA3] mb-3">Recent Sales</p>
          <div className="flex flex-col divide-y divide-[#F0F4F5]">
            {recentSales.map((s) => (
              <div key={s.id} className="py-3 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#2B2B2B] truncate">{s.productName}</p>
                  <p className="text-xs text-[#6F8A91]">{s.quantity} × {fmt$(s.pricePerUnit)} · {fmtDate(s.date)}</p>
                </div>
      ...
      {activeGoals.length > 0 && (
        <Card>
          <p className="text-xs font-bold uppercase tracking-wider text-[#7B9EA3] mb-4">Your Goals</p>
              ...
              return (
                <div key={g.id}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs font-bold text-[#5DB7C4]">{pct}%</span>
                  </div>
                  <div className="h-2.5 rounded-full bg-[#EAF7F9] overflow-hidden">
                    <div className="h-full rounded-full bg-[#5DB7C4] transition-all duration-500" style={{ width: `${pct}%` }} />
                  </div>
...
function InventoryTab() {
  ...
  const AddForm = (
    <div className="rounded-2xl bg-[#F6FCFD] border border-[#C5E9ED] px-4 py-4 flex flex-col gap-3 mb-4">
        ...
        {data.inventory.length === 0 && !showAdd ? (
          ...
          <div className="flex flex-col gap-3">
            {data.inventory.map((item) => {
              ...
              if (editId === item.id) {
                return (
                  <Card key={item.id} className="border-[#5DB7C4]">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-0.5">
                        <p className="text-xs text-[#6F8A91]">Made: <strong className="text-[#2B2B2B]">{item.quantityMade}</strong></p>
                        <p className="text-xs text-[#6F8A91]">Sold: <strong className="text-[#2B2B2B]">{item.quantitySold}</strong></p>
                        <p className="text-xs text-[#6F8A91]">Left: <strong className={remaining <= 0 ? "text-red-500" : remaining <= 3 ? "text-amber-600" : "text-green-600"}>{Math.max(0, remaining)}</strong></p>
                        {item.costPerItem !== "" && <p className="text-xs text-[#6F8A91]">Cost: <strong>{fmt$(Number(item.costPerItem))}</strong></p>}
                        {item.sellingPrice !== "" && <p className="text-xs text-[#6F8A91]">Price: <strong>{fmt$(Number(item.sellingPrice))}</strong></p>}
                      </div>
                    </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        {showForm && (
          <div className="rounded-2xl bg-[#F6FCF0] border border-[#B8E6C0] px-4 py-4 flex flex-col gap-3 mb-4">
        ...
        {sortedSales.length === 0 ? (
          ...
          <Card>
            <p className="text-xs font-bold uppercase tracking-wider text-[#7B9EA3] mb-3">Sale History</p>
            <div className="flex flex-col divide-y divide-[#F0F4F5]">
              {sortedSales.map((s) => (
                <div key={s.id} className="py-3 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#6F8A91] mt-0.5">{fmtDate(s.date)} · {s.quantity} × {fmt$(s.pricePerUnit)}</p>
                    {s.note && <p className="text-xs text-[#9BBFC3] mt-0.5 italic">"{s.note}"</p>}
                  </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        {showForm && (
          <div className="rounded-2xl bg-[#FFF8F0] border border-[#FFD9B0] px-4 py-4 flex flex-col gap-3 mb-4">
        ...
        {sortedExpenses.length === 0 ? (
          ...
          <Card>
            <p className="text-xs font-bold uppercase tracking-wider text-[#7B9EA3] mb-3">Expense Log</p>
            <div className="flex flex-col divide-y divide-[#F0F4F5]">
              {sortedExpenses.map((e) => (
                <div key={e.id} className="py-3 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#6F8A91] mt-0.5">{fmtDate(e.date)} · {e.category}{e.note ? ` · ${e.note}` : ""}</p>
                  </div>
                ...
                <Card key={evt.id}>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-[#6F8A91]">
                      ...
                      {evt.eventGoal && <p className="mt-1.5 flex items-center gap-1 text-xs font-medium text-[#5DB7C4]"><Target className="h-3.5 w-3.5" /> Goal: {evt.eventGoal}</p>}
                      {evt.boothFee !== "" && <p className="text-xs text-[#6F8A91]">Booth fee: {fmt$(Number(evt.boothFee))}</p>}
                    </div>
                ...
                <Card key={evt.id} className="opacity-90">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#6F8A91]">{fmtDate(evt.date)}{evt.location ? ` · ${evt.location}` : ""}</p>
                    </div>
                    {confirmDeleteId === evt.id ? (
                      <DeleteConfirm id={evt.id} onConfirm={(id) => { deleteEvent(id); setConfirmDeleteId(null); }} onCancel={() => setConfirmDeleteId(null)} />
                    ) : (
                  ...
                  {(rev > 0 || exp > 0) && (
                    <div className="flex flex-wrap gap-3 mt-2 rounded-xl bg-[#F9FAFB] border border-[#E5E8EC] px-3 py-2">
                      <p className="text-xs text-[#6F8A91]">Revenue: <strong className="text-[#2B2B2B]">{fmt$(rev)}</strong></p>
                      <p className="text-xs text-[#6F8A91]">Expenses: <strong className="text-[#2B2B2B]">{fmt$(exp)}</strong></p>
                      <p className="text-xs text-[#6F8A91]">Profit: <strong className={profit >= 0 ? "text-green-600" : "text-red-500"}>{fmt$(profit)}</strong></p>
                      {evt.unitsSold !== "" && <p className="text-xs text-[#6F8A91]">Sold: <strong className="text-[#2B2B2B]">{evt.unitsSold} units</strong></p>}
                    </div>
                  )}
                  ...
                  {evt.improvements && <p className="mt-0.5 flex gap-1.5 text-xs text-[#6F8A91]"><Lightbulb className="h-3.5 w-3.5 flex-none text-[#F36C3D]" /> Next time: {evt.improvements}</p>}
                </Card>
              );
            })}
          </div>
...
function GoalsTab() {
  ...
  const goalTypeColors: Record<GoalType, { bar: string; bg: string; text: string }> = {
    ...
    units: { bar: "bg-[#5DB7C4]", bg: "bg-[#EAF7F9]", text: "text-[#5DB7C4]" },
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        {showForm && (
          <div className="rounded-2xl bg-[#F9F6FF] border border-[#DDD0F5] px-4 py-4 flex flex-col gap-3 mb-4">
                ...
                <Card key={g.id}>
                  ...
                  <div className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                      <p className="flex items-center gap-1.5 text-xs text-[#6F8A91]"><Flag className="h-3.5 w-3.5" /> {text}</p>
                      <span className={`text-sm font-extrabold ${pct >= 100 ? "text-green-600" : colors.text}`}>{pct}%</span>
                    </div>
                    <div className="h-3 rounded-full bg-[#F0F4F5] overflow-hidden">
                      <div
...
const INSIGHT_STYLES: Record<InsightType, { card: string; label: string; dot: string }> = {
  ...
  info:    { card: "bg-[#EAF7F9] border-[#C5E9ED]", label: "text-[#5DB7C4]", dot: "bg-[#5DB7C4]" },
      ...
      <div className="mt-2">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-[#6F8A91] mt-0.5">Notes from customers at events or in person</p>
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <p className="text-xs text-[#9BBFC3]">{fmtDate(f.date)}</p>
                    </div>
...
export default function TrackerPage() {
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="sticky top-0 z-20 flex items-center border-b border-[#DCE9EC] bg-white/90 px-4 py-4 shadow-sm backdrop-blur">
        <button
          type="button"
          onClick={() => navigate("/results")}
          className="min-w-[80px] cursor-pointer text-left text-sm font-semibold text-[#5DB7C4] transition-colors hover:text-[#F36C3D]"
        >
          Results
        </button>
        ...
        <p className="min-w-[80px] text-right text-[10px] font-medium leading-tight text-[#9BBFC3]">
          Saved on<br />this device
        </p>
      </header>

      {/* Tab nav */}
      <div className="sticky top-[73px] z-10 border-b border-[#DCE9EC] bg-white/90 shadow-sm backdrop-blur">
              ...
              return (
                <button
                  ...
                  className={`flex min-w-[92px] flex-shrink-0 cursor-pointer items-center justify-center gap-2 rounded-2xl border px-3 py-2 transition-all ${
                    isActive
                      ? "border-[#5DB7C4] bg-[#F0FAFB] text-[#2B2B2B] shadow-sm"
                      : "border-transparent bg-transparent text-[#7B9EA3] hover:border-[#DCE9EC] hover:bg-white"
                  }`}
                >
                  <Icon className={`h-4 w-4 ${isActive ? "text-[#5DB7C4]" : ""}`} />
